...
public:
	...
	inline const Type* operator[] (int r) const {
		assert(r >= 0 && r < N);
		const Type* row = data[r];
		return row;
	}
	...
	friend Matrix<Type, N> operator! (const Matrix<Type, N>& B) {
		// Inversion using Gaussian elimination
		int i, j, k;
		Type a1, a2;
...
const Matrix<T, N> Matrix<T, N>::I = Matrix<T, N>();
...
public:
	static const Vector<Type, N> ZERO;
	...
	void zero() { for (int i = 0; i < N; i++) data[i] = Type(0); }
	int size() const { return N; }
	void normalize() {
		Type m = mag(*this);
	...
	void clamp01() {
		for (int i = 0; i < N; i++) {
			data[i] = min(Type(1), max(Type(0), data[i]));
		}
	}
	...
	friend Vector<Type, N> operator/ (const Vector<Type, N>& U, Type alpha) {
		Vector<Type, N> V;
		for (int i = 0; i < N; i++)
			V.data[i] = U.data[i] / alpha;
		return move(V);
	}
	...
	Vector<Type, N> scale(const Vector<Type, N>& V) const {
		Vector<Type, N> W;
		for (int i = 0; i < N; i++)
