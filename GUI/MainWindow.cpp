...
Geometry* MainWindow::_selected = NULL;
...
void MainWindow::normalKey(unsigned char key, int x, int y) {
	...
	if (key == 'n') {
		if (!objectReady) {
			...
			cout << "Please corresponding number for a shape: " << endl;
			cout << "1. Sphere" << endl;
			cout << "2. Box" << endl;
			cout << "3. Ellipsoid" << endl;
		...
		// TODO: Complete code for object insertion
		// Hints:
		// - initialize object properties
		// - See how SceneUtils::readScene() works
		// - call prepScene() after initializing the object and adding it to the scene
		...
		if (key == '1')
			...
			cout << "Invalid number key entered" << endl;
			return;
		}
	...
	if (key == 'm' && _inputMode == INPUT_EDITING) {
		// TODO: Complete code for object removal
		// Hints:
		// - reset all necessary variables
		// - call prepScene() after successful removal
		if (_selected == NULL) {
			cout << "Select an object, then press m." << endl;
		}
		else {
			_scene->removeObject(_selected);
			_selected = NULL;
			PropertyWindow::closePropertyWindow();
			prepScene();
		}
