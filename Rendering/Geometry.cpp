...
Vec3 GeometryUtils::reflect(const Vec3& v, const Vec3& n) {
	return v - 2 * (v * n) * n;
}

Vec3 GeometryUtils::refract(const Vec3& v, const Vec3& n, double n1, double n2) {
	double dot = v * n;
	double dot2 = dot * dot;
	double f = n1 / n2;
	double f2 = f * f;

	double rp2 = f2 * (1 - dot2);
	...
	double rn = sqrt(1 - rp2);

	return f * v - (f * dot + rn) * n;
}
